...
* **COLOUR IS NOT DIFFERENTIATION (mandatory):** When a set of sibling visual objects must read as *distinct* (icons, avatars, illustrations, 3D props, cards-with-imagery, game buildings), differentiate them by **form / silhouette** — outline, proportion, attached structure, props — not by hue. A palette applied to same-shaped objects still reads as sameness. Grayscale test: strip colour (squint / desaturate) and each element must still read as its own shape. If they collapse into identical blobs without colour, the differentiation is fake — change the forms, not the swatches.
